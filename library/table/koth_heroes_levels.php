...
class Table_LIB_Koth_Heroes_Levels extends Table_LIB_Origin {
    ...
    protected function getInitScript()
    {
        ...
        $bonuses = array( array( 'primary' => 0,     'secondary' => 0,     'ternary' => 0,     'fournary' => 0 ),
                          ...
                          array( 'primary' => 3 + 1, 'secondary' => 2 + 0, 'ternary' => 1 + 0, 'fournary' => 0 ),
                          array( 'primary' => 3 + 2, 'secondary' => 2 + 0, 'ternary' => 1 + 0, 'fournary' => 0 ),
                          array( 'primary' => 3 + 2, 'secondary' => 2 + 1, 'ternary' => 1 + 0, 'fournary' => 0 ),
                          array( 'primary' => 3 + 3, 'secondary' => 2 + 1, 'ternary' => 1 + 0, 'fournary' => 0 ),
                          array( 'primary' => 3 + 3, 'secondary' => 2 + 2, 'ternary' => 1 + 0, 'fournary' => 0 ),
                          array( 'primary' => 3 + 3, 'secondary' => 2 + 2, 'ternary' => 1 + 1, 'fournary' => 0 ) );

        $query = 'INSERT INTO koth_heroes_levels ( id_hero, level, start_hp, max_attack, max_health, max_experience, max_magic, picture ) VALUES ';
        ...
        foreach ( $heroes as $hero )
        {
            foreach( $bonuses as $bonus )
            {
                ...
                $picture        = $this->getQuotedValue( $id_hero . '_' . $level . '_pic' );

                $values[] = "( $id_hero, $level, $start_hp, $max_attack, $max_health, $max_experience, $max_magic, $picture )";
            }
        }
